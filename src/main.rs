...
fn log_candidate(tag: &str, arb: &Arbitrage, mint_in: &Pubkey) {
    ...
    info!("  gross_profit: {} (raw {})", arb.gross_profit, arb.gross_profit_raw);
    info!("  total_fees: {} (raw {})", arb.total_fees, arb.total_fees_raw);
...
fn main() -> Result<()> {
    ...
    if let Some(Command::Config { cmd }) = cli.cmd {
        match cmd {
            ...
            ConfigCmd::ResetDefaults => {
                ...
                println!("State reset to defaults and saved to {}", state_path.display());
            }
            ...
            ConfigCmd::SetSpreadThresholdBps => config_set_spread_threshold_bps(&state_path, &mut state)?,
    ...
    let spread_threshold_bps = take_or_panic(cli.spread_threshold_bps, state.spread_threshold_bps, "spread-threshold-bps");
    let slippage_bps = take_or_panic(cli.slippage_bps, state.slippage_bps, "slippage-bps");
    let priority_fee_microlamports = take_or_panic(cli.priority_fee, state.priority_fee_microlamports, "priority-fee");
    ...
    let mint_in = state.mint_in.as_ref().expect("mint-in is required").parse::<Pubkey>()?;
    let mint_out = state.mint_out.as_ref().expect("mint-out is required").parse::<Pubkey>()?;
    ...
    let pool_a = PoolData::new(&rpc, &pool_a_addr, &decoder)
        .map_err(|e| { error!("RPC error loading PoolA {}: {}", pool_a_addr, e); e })?;
    let pool_b = PoolData::new(&rpc, &pool_b_addr, &decoder)
        .map_err(|e| { error!("RPC error loading PoolB {}: {}", pool_b_addr, e); e })?;

    // Raw values, then normalized so that token0 == mint_in for BOTH pools
    let mut pool_a_values = pool_a.get_values(&rpc)
        .map_err(|e| { error!("RPC error fetching PoolA values: {}", e); e })?;
    let mut pool_b_values = pool_b.get_values(&rpc)
        .map_err(|e| { error!("RPC error fetching PoolB values: {}", e); e })?;
    ...
    let rent_per_ata = get_token_account_rent(&rpc)
        .map_err(|e| { error!("RPC error fetching token account rent: {}", e); e })?;
    ...
    info!("  {} → ATA: {}  (exists: {})", pk_s(&mint_in), ata_in_addr, atas[0].exists);
    info!("  {} → ATA: {}  (exists: {})", pk_s(&mint_out), ata_out_addr, atas[1].exists);
    ...
    step!(
        ...
        ata_in_addr, atas[0].exists, ata_out_addr, atas[1].exists, rent_per_ata, rent_raw
    );

    // ---------- PnL both directions ----------
    let arb_a_b = calculate_pnl(amount_in, &pool_a_values, &pool_b_values, rent_raw, priority_fee_microlamports);
    let arb_b_a = calculate_pnl(amount_in, &pool_b_values, &pool_a_values, rent_raw, priority_fee_microlamports);
    ...
    let (arb_chosen, first_label, second_label, pool_in, pool_out, in_vals, out_vals, price_first, price_second) =
        choose_direction(&arb_a_b, &arb_b_a, &pool_a, &pool_b, &pool_a_values, &pool_b_values, price_a, price_b);
    ...
    info!("  After first swap ({}): {} (mint_out {})", pool_in.pool_id, out1, pk_s(&mint_out));
    info!("  After second swap ({}): {} (mint_in {})", pool_out.pool_id, out2, pk_s(&mint_in));

    step!(steps, "Direction: first={} ({}), second={} ({})",
        first_label, pool_in.pool_id, second_label, pool_out.pool_id);
    step!(steps, "Flow: out1={} {}, out2={} {}", out1, pk_s(&mint_out), out2, pk_s(&mint_in));

    // ---------- Decision ----------
    let is_profitable = if let Some(p) = arb_chosen.pnl { p > 0.0 } else { arb_chosen.gross_profit > 0.0 };
    ...
    if !is_profitable {
        warn!("Not profitable (pnl {:?}, gross {})", arb_chosen.pnl, arb_chosen.gross_profit);
        step!(steps, "Not profitable");
    }
    if !meets_spread_threshold {
        warn!("Spread below threshold: {:.4} < {}", spread_bps_val, spread_threshold_bps);
        step!(steps, "Spread below threshold: {:.4} < {}", spread_bps_val, spread_threshold_bps);
    }
    ...
    info!("Slippage protection: min_out(raw)={} (slippage_bps={})", min_out, slippage_bps);
    step!(steps, "min_out (slippage_bps={}) = {}", slippage_bps, min_out);

    let tx = create_arbitrage_transaction(
        &rpc, &keypair, pool_in, pool_out,
        arb_chosen.amount_in_raw, arb_chosen.amount_out_1_raw,
        atas.clone(), min_out, priority_fee_microlamports,
    ).map_err(|e| { error!("Error building transaction: {}", e); e })?;
